...
func TestDrawContainImageAlphaBlendsSource(t *testing.T) {
	dst := image.NewRGBA(image.Rect(0, 0, 1, 1))
	dst.SetRGBA(0, 0, color.RGBA{R: 10, G: 20, B: 30, A: 255})
	...
	DrawImage(dst, src, image.Rect(0, 0, 1, 1), ImageFitContain)
...
func TestDrawContainImageKeepsDestinationForTransparentSource(t *testing.T) {
	dst := image.NewRGBA(image.Rect(0, 0, 1, 1))
	dst.SetRGBA(0, 0, color.RGBA{R: 10, G: 20, B: 30, A: 255})
...
func TestDrawContainImageRespectsDestinationOffset(t *testing.T) {
	dst := image.NewRGBA(image.Rect(0, 0, 4, 4))
	src := image.NewRGBA(image.Rect(0, 0, 2, 2))
	...
	DrawImage(dst, src, image.Rect(1, 1, 3, 3), ImageFitContain)
...
func TestDrawContainImageClipsToDestinationBounds(t *testing.T) {
	dst := image.NewRGBA(image.Rect(10, 20, 13, 23))
	src := image.NewRGBA(image.Rect(0, 0, 3, 3))
	...
	DrawImage(dst, src, image.Rect(12, 22, 15, 25), ImageFitContain)
